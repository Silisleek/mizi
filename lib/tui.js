...
import { createSession, saveSession, loadSession, listSessions, deleteSession, formatSessionPreview } from './sessions.js';
...
function SessionPicker({ sessions, onSelect, onDelete, onCancel }) {
  ...
  useInput((input, key) => {
    ...
    else if (key.return && sessions[cursor]) onSelect(sessions[cursor]);
    else if ((key.delete || input === 'x') && sessions[cursor]) onDelete(sessions[cursor].id);
    else if (key.escape) onCancel?.();
  });

  if (!sessions.length) {
    return h(Box, { flexDirection: 'column', paddingLeft: 1 },
      h(Text, { color: P.lav, bold: true }, '  No saved sessions'),
      h(Text, { color: P.dim }, '  Press Esc to go back'),
    );
  }

  return h(Box, { flexDirection: 'column', paddingLeft: 1 },
    h(Text, { color: P.lav, bold: true }, '  Past sessions'),
    h(Text, { color: P.dim }, '  ↓↑ select · Enter load · x delete · Esc cancel'),
    h(Text, null, ''),
    ...sessions.map((s, i) => {
      const active = i === cursor;
      return h(Text, { key: s.id },
        active ? h(Text, { color: P.green }, '▸ ') : h(Text, null, '  '),
        h(Text, { color: active ? P.text : P.dim, bold: active }, (s.title || '(untitled)').padEnd(36)),
        h(Text, { color: P.dim }, s.model ? s.model.split('/').pop() + '  ' : ''),
        h(Text, { color: P.dim }, `${s.msgCount} msgs  ${s.timeAgo}`),
      );
    }),
  );
}
...
function getSystemPrompt() {
  ...
  const providerLabel = provider?.name || 'unknown provider';
  return `You are a helpful AI coding assistant running in the user's terminal.
You are running as ${modelLabel} via ${providerLabel}.
...
function App() {
  ...
  const handleSlash = useCallback(async (text) => {
    ...
    switch (cmd) {
      ...
      case '/model': {
        if (arg) { config.set('model', arg); setOutputLines([`  ✓ Model → ${arg}`]); break; }
        // Fetch models from provider and show arrow-key picker
        try {
          const prov = getActiveProvider();
      ...
      case '/sessions': {
        ...
        const previews = sessions.map(formatSessionPreview);
        setSessionPicker(previews);
        setOutputLines([]);
        break;
      }
  ...
  const handleSubmit = useCallback(async (text) => {
    ...
    try {
      let full = '';
      for await (const chunk of streamChat([{ role: 'system', content: getSystemPrompt() }, ...newMsgs.slice(-20)])) {
      ...
      if (toolMatch) {
        ...
        let f2 = '';
        for await (const c of streamChat([{ role: 'system', content: getSystemPrompt() }, ...updated.slice(-20)])) { f2 += c; setStreamBuffer(f2); }
    ...
    return h(SessionPicker, {
      ...
      onSelect: (s) => {
        ...
        if (session) {
          ...
          if (session.model) config.set('model', session.model);
          if (session.provider) config.set('provider', session.provider);
        }
      ...
      onDelete: (id) => {
        deleteSession(id);
        setSessionPicker(listSessions().map(formatSessionPreview));
      },
